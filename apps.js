// ============================================================
//  f0rLorN Apps Config — managed via panel
// ============================================================

const APPS = [
  {
    name: "Audio",
    tag: "Audio Processing",
    desc: "f0rLorN Audio is a free Windows app that makes your mic sound professional — noise gate, EQ, compressor, all in one. No subscription. No bloat. Just plug in and sound better.",
    downloadUrl: "https://github.com/f0rlorN1/f0rlorN-Audio/releases/tag/v1.0.0",
    icon: "audio",
    badge: "Free",
    status: "released",
  },
  {
    name: "f0rlorN Cleaner",
    tag: "System Cleaning",
    desc: "f0rlorN Cleaner is a free Windows app that keeps your PC clean and fast — junk files, duplicates, browser cache, startup manager, all in one. No subscription. No bloat. Just run and free up space.",
    downloadUrl: "https://github.com/f0rlorN1/f0rlorN-Cleaner-Release/releases/tag/v1.0.0",
    icon: "audio",
    badge: "Free",
    status: "released",
  },
];
